<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="home.aspx.vb" Inherits="DashBoardClinicManagmmantSystem.home" %>
        ...
        * {
        ...
        #talk {
            ...
            #talk .col-50:nth-child(2) {
                background: url('https://stage.webpsychology.com/sites/default/files/article_images/shutterstock_84259324.jpg') topcover no-repeat;
            }
...
<body>
    ...
    <section id="how-it-works">
        <div class="container">
            <div class="intro">
                <a href="AddClinic.aspx" class="btn-link">
                    <span>سجل الآن <i class="fa fa-arrow-right"></i></span>
                </a>
                <%--<span>سجل الآن <i class="fa fa-arrow-right"></i></span>--%>
                هل أنت طبيب أو صاحب عيادة؟
            </div>
            <div class="slug">التسجيل سهل وسريع، ويتيح لك الوصول لكل أدواتك الطبية</div>
            <div class="title">خدماتنا لك وللمرضى</div>
            <div class="content">
                - للمرضى: ابحث عن عيادة أو طبيب بالقرب منك<br>
                - للطبيب: إدارة العيادات والمواعيد بسهولة عبر داشبورد احترافي<br>
                - دليل الأطباء: صفحتك الاحترافية تعرض خبراتك وخدماتك<br>
                - تواصل فعال مع المرضى وحافظ على سجلات دقيقة
            </div>
        </div>
    </section>

    <ul class="staff" runat="server" id="staffContainer">
        
    </ul>
    <%--<div class="card" aria-hidden="true">
        <div class="card__exit"></div>
    ...
    <button class="btn btn--primary">زيارة الموقع</button>
    <i class="exit material-icons">close</i>
</div>
    ...
    <section id="question">
        <div class="title">اعثر على عيادتك المثالية</div>
        <div class="content">
            استخدم محرك البحث السريع للعثور على أفضل العيادات والأطباء بالقرب منك، 
        مع الاطلاع على تقييماتهم وساعات العمل والخدمات المتوفرة.
        </div>
        <div class="form">
    <input type="text" id="searchInput" placeholder="ابحث عن طبيب أو عيادة">
    ...
    <section id="talk">
        <div class="col-wrapper">
            <div class="col-50">
                <div class="title">فوائد الانضمام كطبيب</div>
                <div class="content">
                    عند تسجيلك كطبيب أو عيادة، ستحصل على لوحة تحكم احترافية لإدارة العيادة
                    <br />
                    صفحة احترافية في دليل الأطباء، وإمكانية التواصل مع المرضى بسهولة وأمان
                </div>
                <div class="options">
                    <ul>
                        <li><span><i class="fa fa-check"></i></span>لوحة تحكم كاملة للعيادة</li>
                        <li><span><i class="fa fa-check"></i></span>صفحة احترافية في دليل الأطباء</li>
                        <li><span><i class="fa fa-check"></i></span>تواصل آمن وسريع مع المرضى</li>
                    </ul>
                </div>
            </div>
            <div class="col-50">
                <img src="Images/Unt1234555itled.png" alt="Doctor Dashboard" style="width: 100%; max-width: 400px;">
            </div>
        </div>
    </section>
    ...
    <section id="signup">
        <div class="slug">هل أنت جاهز للانضمام؟</div>
        <div class="title">ابدأ الآن وارتقِ بعيادتك</div>
        <div class="content">
            انضم كطبيب للحصول على داشبورد كامل،
            دليل الأطباء، صفحة احترافية، وإدارة مواعيدك بكل سهولة.
        </div>
        <div class="button-wrapper">
            <button class="highlight">سجل الآن</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // عند الضغط على Enter في البحث، اذهب لصفحة البحث
            searchInput.addEventListener('keydown', (e) => {
                if (e.key === 'Enter') {
                    const query = searchInput.value.trim();
                    if (query) {
                        // تحويل النص إلى URL صالح
                        const encodedQuery = encodeURIComponent(query);
                        // الذهاب لصفحة البحث مع إرسال قيمة البحث
                        window.location.href = `SearchPage.aspx?query=${encodedQuery}`;
                    }
                }
            });

            // اختياري: الضغط على أيقونة البحث
            const searchIcon = document.querySelector('#question .fa-search');
            if (searchIcon) {
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // عند الضغط على أي عنصر <li>، فتح الرابط مباشرة
        staffContainer?.addEventListener('click', (e) => {
            const staffMember = e.target.closest('li');
            if (!staffMember) return;

            const clinicId = staffMember.dataset.id;
            if (clinicId) {
                // فتح صفحة تفاصيل العيادة مباشرة
                window.location.href = `clinicdetails.aspx?clinicId=${clinicId}`;
            }
        });
        ...
        const applyFilters = () => {
            ...
            staffContainer?.querySelectorAll('li').forEach(staffMember => {
                const name = staffMember.querySelector('.name')?.textContent.toLowerCase() || '';
                const title = staffMember.querySelector('.title')?.textContent.toLowerCase() || '';

                const matchesSearch = name.includes(searchText);
                const matchesTitle = !titleText || title === titleText;
            ...
            if (staffContainer) {
                staffContainer.scrollIntoView({ behavior: 'smooth', block: 'start' });
            }
        };


        if (searchInput) searchInput.addEventListener('input', applyFilters);
        if (titleFilter) titleFilter.addEventListener('change', applyFilters);
